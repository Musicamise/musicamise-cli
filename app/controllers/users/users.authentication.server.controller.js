 ...
 */
exports.saveOAuthUserProfile2 = function(req, providerUserProfile, done) {
	if (!req.user) {
...
exports.saveOAuthUserProfile = function(req, providerUserProfile, done) {
	if (!req.user) {
		...
		// Define a search query to find existing user with current provider profile
		var searchQuery = {
			$or: [mainProviderSearchQuery, additionalProviderSearchQuery,{email:providerUserProfile.email}]
		};

		User.findOne(searchQuery,'-password -salt', function(err, user) {
			if (err) {
				return done(err);
			} else {
				if (!user) {
					...
					user = new User({
						firstName: providerUserProfile.firstName,
						lastName: providerUserProfile.lastName,
						fullName: providerUserProfile.fullName,
						// username: availableUsername,
						displayName: providerUserProfile.displayName,
						email: providerUserProfile.email,
						provider: providerUserProfile.provider,
